...
public class AccountDAOImpl implements AccountDAO {
	...
	private static Logger log = Logger.getLogger(AccountDAOImpl.class.getCanonicalName());
		...
		try {
			...
			while (rs.next()) {
				...
				if (log.isLoggable(Level.FINEST))
					log.finest("getAllAccounts(): Get  Account " + acc);
				allAccounts.add(acc);
			}
			return allAccounts;
	...
	public Account getAccountById(long accountId) throws CustomException {
		...
		try {
			...
			if (rs.next()) {
				...
				if (log.isLoggable(Level.FINEST))
					log.finest("Retrieve Account By Id: " + acc);
			}
			return acc;
		} catch (SQLException e) {
			throw new CustomException("getAccountById(): Error reading account data", e);
	...
	public long createAccount(Account account) throws CustomException {
		...
		try {
			...
			if (affectedRows == 0) {
				log.severe("createAccount(): Creating account failed, no rows affected.");
			...
			if (generatedKeys.next()) {
				...
				log.severe("Creating account failed, no ID obtained.");
				throw new CustomException("Account Cannot be created");
			}
		} catch (SQLException e) {
			log.severe("Error Inserting Account  " + account);
		...
		try {
			...
			if (rs.next()) {
				targetAccount = new Account(rs.getLong("AccountId"), rs.getString("UserName"),
						rs.getBigDecimal("Balance"), rs.getString("CurrencyCode"));
				if (log.isLoggable(Level.FINEST))
					log.finest("updateAccountBalance from Account: " + targetAccount);
			}
			...
			conn.commit();
			if (log.isLoggable(Level.FINEST))
				log.finest("New Balance after Update: " + targetAccount);
			return updateCount;
		} catch (SQLException se) {
			// rollback transaction if exception occurs
			log.severe("updateAccountBalance(): User Transaction Failed, rollback initiated for: " + accountId + se.getLocalizedMessage());
			try {
				if (conn != null)
					conn.rollback();
		...
		try {
			...
			if (rs.next()) {
				fromAccount = new Account(rs.getLong("AccountId"), rs.getString("UserName"),
						rs.getBigDecimal("Balance"), rs.getString("CurrencyCode"));
				if (log.isLoggable(Level.FINEST))
					log.finest("transferAccountBalance from Account: " + fromAccount);
			}
			lockStmt = conn.prepareStatement(SQL_LOCK_ACC_BY_ID);
			...
			if (rs.next()) {
				toAccount = new Account(rs.getLong("AccountId"), rs.getString("UserName"), rs.getBigDecimal("Balance"),
						rs.getString("CurrencyCode"));
				if (log.isLoggable(Level.FINEST))
					log.finest("transferAccountBalance to Account: " + toAccount);
			}
			...
			if (log.isLoggable(Level.FINEST)) {
				log.finest("Number of rows updated for the transfer : " + result);
			}
			// If there is no error, commit the transaction
			conn.commit();
		} catch (SQLException se) {
			// rollback transaction if exception occurs
			log.severe("transferAccountBalance(): User Transaction Failed, rollback initiated for: " + userTransaction + se.getLocalizedMessage());
			try {
				if (conn != null)
					conn.rollback();
			} catch (SQLException re) {
